...
import { Loader2, Trash2, Calculator, Layers, HardHat, Tractor, PieChart, ChevronDown, ChevronRight, FoldVertical, Download, RefreshCw, CheckCircle2, Radar, Link2 } from "lucide-react";
...
import { deleteEstimationItem, createManualEstimationItem, updateEstimationQuantity, updateEstimationPriceByGroup, updateEstimationMaterialByGroup } from "@/lib/action/estimationActions";
...
export default function ProjectEstimationTab({ projectId, onUpdate }: Props) {
    ...
    const loadData = async () => {
        ...
        // NGUỒN 2: QUÉT TRỰC TIẾP VÀO LÕI GÓI THẦU
    ...
    const handleRateChange = async (id: string, category: string, newRate: string) => {
        ...
        if (id.startsWith('temp-')) {
            const data = {
                material_name: category === 'GT' ? 'Chi phí chung' : category === 'LN' ? 'Lợi nhuận' : 'VAT',
                category: category,
                unit: '%',
                quantity: rate,
                unit_price: 0
            };
            res = (await createManualEstimationItem(projectId, data)) as ActionResponse;
        } else {
            res = (await updateEstimationQuantity(id, projectId, rate, category)) as ActionResponse;
        }
    ...
    // ==========================================
    // TÍNH NĂNG XUẤT EXCEL CHUYÊN NGHIỆP CÓ ĐỊNH DẠNG (BORDERS)
    // ==========================================
    const handleExportDetailExcel = async () => {
        try {
            ...
            const sheet = workbook.addWorksheet('PhanTichDonGia', {
                views: [{ state: 'frozen', xSplit: 0, ySplit: 1 }]
            });

            // 1. Cấu hình Cột (BỎ CỘT TÊN HẠNG MỤC - Còn 8 cột)
            sheet.columns = [
                { header: 'STT', key: 'stt', width: 8, style: { alignment: { horizontal: 'center', vertical: 'middle' } } },
                { header: 'Mã Định Mức', key: 'code', width: 15, style: { alignment: { horizontal: 'center', vertical: 'middle' } } },
                { header: 'Tên Công Tác / Hao Phí', key: 'name', width: 45, style: { alignment: { wrapText: true, vertical: 'middle' } } },
                { header: 'ĐVT', key: 'unit', width: 10, style: { alignment: { horizontal: 'center', vertical: 'middle' } } },
                { header: 'Khối Lượng', key: 'volume', width: 15, style: { numFmt: '#,##0.000', alignment: { horizontal: 'right', vertical: 'middle' } } },
                { header: 'Định Mức', key: 'norm', width: 12, style: { numFmt: '#,##0.0000', alignment: { horizontal: 'center', vertical: 'middle' } } },
                { header: 'Đơn Giá', key: 'price', width: 15, style: { numFmt: '#,##0', alignment: { horizontal: 'right', vertical: 'middle' } } },
                { header: 'Thành Tiền', key: 'total', width: 18, style: { numFmt: '#,##0', alignment: { horizontal: 'right', vertical: 'middle' } } }
            ];

            // 2. Format Tiêu Đề Dòng 1 (In đậm, Canh giữa, Kẻ khung)
            const headerRow = sheet.getRow(1);
            headerRow.font = { bold: true, name: 'Times New Roman', size: 12, color: { argb: 'FFFFFFFF' } };
            ...
            // 3. Đổ dữ liệu Data Rows
            let stt = 1;

            sections.forEach((section, secIdx) => {
                const tasks = qtoTasks.filter(i => i.parent_id === section.id && i.item_type === 'task');
                if (tasks.length === 0) return;

                // ---> DÒNG TÊN HẠNG MỤC
                const secRow = sheet.addRow({
                    stt: toRoman(secIdx + 1),
                    code: section.item_name, // Ghi tên hạng mục vào cột B (Mã Định Mức) để lát nữa gộp ô từ B -> H
                    name: '', unit: '', volume: '', norm: '', price: '', total: ''
                });
                secRow.font = { bold: true, name: 'Times New Roman', size: 11 };
                secRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFEFEFEF' } };

                // Lệnh Merge Cell từ Cột 2 (B) đến Cột 8 (H) của dòng Hạng mục này
                sheet.mergeCells(secRow.number, 2, secRow.number, 8);
                // Canh giữa text cho dải ô đã Merge
                secRow.getCell(2).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
                ...
                tasks.forEach((task) => {
                    ...
                    const tRow = sheet.addRow({
                        stt: stt++,
                        code: task.norm_code || '',
                        name: task.item_name, // Chữ sẽ wrap theo cấu hình cột mặc định
                        unit: task.unit,
                        volume: taskVol,
                        norm: '',
                        price: taskUnitPrice,
                    ...
                    tRow.font = { bold: true, name: 'Times New Roman', size: 11 };
                    tRow.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFCE4D6' } }; // Vàng nhạt

                    // ---> DÒNG HAO PHÍ (VL, NC, M)
                    ['VL', 'NC', 'M'].forEach(cat => {
                        ...
                        // Dòng Tiêu đề nhóm hao phí (Ghi vào cột Name - Cột 3)
                        const catRow = sheet.addRow({ name: catName });
                        catRow.font = { italic: true, bold: true, color: { argb: 'FFC00000' }, name: 'Times New Roman' };

                        // Dòng Chi tiết hao phí con
                        items.forEach(hp => {
                            ...
                            const hpRow = sheet.addRow({
                                code: hp.material_code || '',
                                name: `- ${hp.material_name}`,
                                unit: hp.unit,
                                volume: rawQty,
                                norm: normVal,
                            ...
                            hpRow.font = { name: 'Times New Roman', size: 11 };
                        });
                    });
                });
            });

            // 4. RULE KẺ BẢNG TỪ A1 -> H(n): Kẻ tất cả các ô kể cả ô trống (8 Cột)
            ...
            // 5. Build File Blob và Tải xuống
            const buffer = await workbook.xlsx.writeBuffer();
            const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
            saveAs(blob, `PhanTichDonGia_${projectId}.xlsx`);
    ...
    return (
        <div className="animate-in fade-in space-y-6 transition-colors duration-500">
            {/* THÔNG BÁO POPUP ĐỒNG BỘ GIÁ */}
            <Dialog open={showSyncPopup} onOpenChange={setShowSyncPopup}>
                <DialogContent className="max-w-3xl shadow-2xl dark:border-slate-800 dark:bg-slate-900">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2 text-xl font-bold text-emerald-700 dark:text-emerald-500">
                            <CheckCircle2 className="h-6 w-6" /> Có cập nhật giá mới từ Phòng Thu Mua!
                        </DialogTitle>
                    </DialogHeader>
                    <div className="py-2 text-slate-700 dark:text-slate-300">
                        ...
                        <div className="custom-scrollbar max-h-[50vh] overflow-y-auto rounded-md border border-slate-200 dark:border-slate-800">
                            <Table>
                                <TableHeader className="sticky top-0 z-10 bg-slate-100 shadow-sm dark:bg-slate-800">
                                    <TableRow>
                                    ...
                                    {diffs.map((d, i) => {
                                        ...
                                        return (
                                            <TableRow key={i} className="dark:border-slate-800">
                                                ...
                                                <TableCell className="text-right text-slate-400 line-through dark:text-slate-500">{formatCurrency(d.old_price)}</TableCell>
                    ...
                    <DialogFooter className="mt-2 border-t pt-4 dark:border-slate-800">
                        ...
                        <Button onClick={handleSyncPrices} disabled={isSyncing} className="min-w-[150px] bg-emerald-600 text-white hover:bg-emerald-700">
            ...
            {/* HEADER TOOLBAR */}
            <div className="flex flex-col items-start justify-between gap-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition-colors xl:flex-row xl:items-center dark:border-slate-800 dark:bg-slate-900">
                <div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                    <Button
                        ...
                        disabled={isSyncing}
                        className="h-9 border-blue-200 bg-blue-50 font-bold text-blue-700 shadow-sm hover:bg-blue-100 dark:border-blue-900/50 dark:bg-blue-900/20 dark:text-blue-400"
                    >
                    ...
                    >
                        <Radar className="mr-2 h-4 w-4" /> Dò Giá Thu Mua
                    </Button>

                    <Button
                        onClick={handlePushToProcurement}
                        className="ml-1 h-9 bg-indigo-600 font-bold text-white shadow-sm hover:bg-indigo-700"
                    >
                        <Tractor className="mr-2 h-4 w-4" /> Chốt dự toán & Gửi
                    </Button>
                </div>
            </div>

            {/* BẢNG TÍNH TOÁN THÔNG SỐ KINH PHÍ */}
            <Card className="overflow-hidden border border-blue-200 bg-white shadow-md transition-colors dark:border-blue-900/50 dark:bg-slate-950">
                <div className="flex items-center gap-2 border-b border-blue-100 bg-blue-50 p-3 dark:border-blue-900/30 dark:bg-blue-500/10">
                    <Calculator className="h-5 w-5 text-blue-600 dark:text-blue-400" />
                    <h4 className="text-sm font-bold tracking-wide text-blue-800 uppercase dark:text-blue-300">Bảng Tính Toán Thông Số Kinh Phí</h4>
                    <Button onClick={handleExportDetailExcel} className="ml-auto h-9 border-indigo-200 bg-indigo-50 font-bold text-indigo-700 hover:bg-indigo-100 dark:border-indigo-800 dark:bg-indigo-900/20 dark:text-indigo-400">
                        <Download className="mr-2 h-4 w-4" /> Xuất Excel Phân Tích Giá
                    </Button>
                </div>
                <Table>
                    <TableHeader>
                        <TableRow className="border-b bg-slate-50 dark:border-slate-800 dark:bg-slate-900">
                    ...
                    <TableBody>
                        ...
                        <TableRow className="bg-slate-50/50 dark:border-slate-800 dark:bg-slate-900/30">
                            ...
                            <TableCell className="p-1">
                                <Input
                                    key={`gt_${gtParam.id}_${gtParam.quantity}`}
                                    type="number"
                                    className="h-8 border-blue-200 bg-white text-center font-bold text-blue-700 dark:border-blue-900 dark:bg-slate-950 dark:text-blue-400"
                                    defaultValue={gtParam.quantity}
                                    onBlur={(e) => handleRateChange(gtParam.id, 'GT', e.target.value)}
                                />
                            </TableCell>
                        ...
                        <TableRow className="dark:border-slate-800">
                            ...
                            <TableCell className="p-1">
                                <Input
                                    key={`ln_${lnParam.id}_${lnParam.quantity}`}
                                    type="number"
                                    className="h-8 border-blue-200 bg-white text-center font-bold text-blue-700 dark:border-blue-900 dark:bg-slate-950 dark:text-blue-400"
                                    defaultValue={lnParam.quantity}
                                    onBlur={(e) => handleRateChange(lnParam.id, 'LN', e.target.value)}
                                />
                            </TableCell>
                            <TableCell className="text-right font-medium text-slate-700 dark:text-slate-300">{formatCurrency(TL)}</TableCell>
                        </TableRow>
                        <TableRow className="bg-orange-50/30 dark:border-slate-800 dark:bg-orange-500/5">
                        ...
                        <TableRow className="dark:border-slate-800">
                            ...
                            <TableCell className="p-1">
                                <Input
                                    key={`vat_${vatParam.id}_${vatParam.quantity}`}
                                    type="number"
                                    className="h-8 border-blue-200 bg-white text-center font-bold text-blue-700 dark:border-blue-900 dark:bg-slate-950 dark:text-blue-400"
                                    defaultValue={vatParam.quantity}
                                    onBlur={(e) => handleRateChange(vatParam.id, 'VAT', e.target.value)}
                                />
                            </TableCell>
                        ...
                        <TableRow className="bg-blue-600 dark:bg-blue-700">
                            ...
                            <TableCell colSpan={3} className="text-base font-bold tracking-wider text-white uppercase">Tổng Cộng Chi Phí Xây Dựng (Sau Thuế)</TableCell>
                            <TableCell className="text-right text-lg font-extrabold text-white">{formatCurrency(TotalProject)}</TableCell>
                        </TableRow>
                    </TableBody>
                </Table>
            </Card>
            ...
            <div className="flex flex-wrap items-center gap-2 rounded-md border border-slate-200 bg-slate-100 p-1 transition-colors dark:border-slate-700 dark:bg-slate-800">
                <Button variant="ghost" onClick={() => setActiveModal('total')} className="font-bold text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-900 hover:text-blue-700 transition-all"><PieChart className="mr-2 h-4 w-4 text-blue-600 dark:text-blue-400" /> Bảng Tổng Hợp Chi Phí</Button>
                <div className="mx-1 h-6 w-px bg-slate-300 dark:bg-slate-600"></div>
                <Button variant="ghost" onClick={() => setActiveModal('VL')} className="font-bold text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-900 hover:text-orange-600 transition-all"><Layers className="mr-2 h-4 w-4 text-orange-500" /> Tổng hợp Vật liệu</Button>
                <Button variant="ghost" onClick={() => setActiveModal('NC')} className="font-bold text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-900 hover:text-green-600 transition-all"><HardHat className="mr-2 h-4 w-4 text-green-500" /> Tổng hợp Nhân công</Button>
                <Button variant="ghost" onClick={() => setActiveModal('M')} className="font-bold text-slate-700 dark:text-slate-300 hover:bg-white dark:hover:bg-slate-900 hover:text-purple-600 transition-all"><Tractor className="mr-2 h-4 w-4 text-purple-500" /> Tổng hợp Máy TC</Button>
            </div>

            {/* BẢNG DỰ TOÁN CHI TIẾT */}
            <Card className="overflow-hidden border border-slate-200 bg-white shadow-sm transition-colors dark:border-slate-800 dark:bg-slate-950">
                <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 p-2 transition-colors dark:border-slate-800 dark:bg-slate-900">
                    <h4 className="ml-2 text-sm font-bold tracking-wide text-slate-700 uppercase dark:text-slate-300">Bảng Phân Tích Vật Tư & Chi Phí Trực Tiếp</h4>
                    <Button variant="outline" size="sm" onClick={handleToggleAllSections} className="h-7 border-slate-300 bg-white text-xs text-slate-600 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-400 dark:hover:bg-slate-800">
                ...
                <Table className="bg-white dark:bg-slate-950">
                    <TableHeader>
                        <TableRow className="border-b-2 bg-slate-100 dark:border-slate-800 dark:bg-slate-900">
                            <TableHead className="w-[50px] text-center font-bold text-slate-800 dark:text-slate-200">STT</TableHead>
                            <TableHead className="w-[100px] text-center font-bold text-slate-800 dark:text-slate-200">Mã hiệu</TableHead>
                            <TableHead className="w-[90px] text-center font-bold text-slate-800 dark:text-slate-200">Loại</TableHead>
                    ...
                    <TableBody>
                        {!initLoaded ? (
                            <TableRow><TableCell colSpan={11} className="py-8 text-center"><Loader2 className="text-muted-foreground mx-auto h-6 w-6 animate-spin dark:text-slate-400" /></TableCell></TableRow>
                        ) : (
                            <>
                                {sections.map((section, secIdx) => {
                                    ...
                                    return (
                                        <React.Fragment key={section.id}>
                                            <TableRow className="border-b-2 border-slate-300 bg-slate-200 transition-colors dark:border-slate-700 dark:bg-slate-800">
                                                ...
                                                <TableCell className="p-1" colSpan={10}>
                                                    <div className="flex w-full items-center gap-1">
                                                        <Button variant="ghost" size="sm" className="h-6 w-6 shrink-0 p-0 text-slate-700 hover:bg-slate-300 dark:text-slate-300 dark:hover:bg-slate-700" onClick={() => toggleSection(section.id)}>
                                                            {isSectionExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                                                        </Button>
                                                        <span className="px-2 font-bold tracking-wide text-slate-800 uppercase dark:text-slate-100">{section.item_name}</span>
                                                    </div>
                                                </TableCell>
                                            </TableRow>
                                            ...
                                            {isSectionExpanded && tasks.map((task, taskIdx) => {
                                                ...
                                                return (
                                                    <React.Fragment key={task.id}>
                                                        <TableRow className="border-b border-slate-200 bg-slate-100/50 dark:border-slate-800 dark:bg-slate-900/50">
                                                            <TableCell className="border-r border-slate-200 text-center font-bold text-slate-600 dark:border-slate-800 dark:text-slate-400">{taskIdx + 1}</TableCell>
                                                            <TableCell className="border-r border-slate-200 text-center font-bold text-blue-700 dark:border-slate-800 dark:text-blue-400">{task.norm_code}</TableCell>
                                                            <TableCell className="border-r border-slate-200 dark:border-slate-800"></TableCell>
                                                            <TableCell className="cursor-pointer border-r border-slate-200 font-bold text-blue-900 hover:bg-blue-50/50 dark:border-slate-800 dark:text-blue-300 dark:hover:bg-blue-900/20" onClick={() => toggleRow(task.id)}>
                                                                <div className="flex items-start gap-1">
                                                                    <Button variant="ghost" size="sm" className="mt-0.5 h-5 w-5 shrink-0 p-0 text-blue-700 hover:bg-blue-200 dark:text-blue-400 dark:hover:bg-blue-800">
                                                            ...
                                                            <TableCell className="border-r border-slate-200 text-center font-bold text-slate-700 dark:border-slate-800 dark:text-slate-300">{task.unit}</TableCell>
                                                            <TableCell className="border-r border-slate-200 bg-slate-100/50 dark:border-slate-800 dark:bg-slate-900/50"></TableCell>
                                                            <TableCell className="border-r border-slate-200 bg-slate-100/50 dark:border-slate-800 dark:bg-slate-900/50"></TableCell>
                                                            <TableCell className="border-r border-slate-200 bg-blue-50/50 text-right text-base font-bold text-blue-700 dark:border-slate-800 dark:bg-blue-500/10 dark:text-blue-400">
                                                                {taskVol.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 5 })}
                                                            </TableCell>
                                                            <TableCell className="border-r border-slate-200 bg-indigo-50/30 text-right font-bold text-indigo-700 dark:border-slate-800 dark:bg-indigo-900/20 dark:text-indigo-400">
                                                                {formatCurrency(taskUnitPrice)}
                                                            </TableCell>
                                                            <TableCell className="bg-emerald-50/30 text-right text-base font-bold text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400">
                                                        ...
                                                        {isTaskExpanded && children.map(child => {
                                                            ...
                                                            return (
                                                                <TableRow key={child.id} className={`${child.material_code ? "hover:bg-green-50/50 dark:hover:bg-green-900/10" : "hover:bg-muted/30 dark:hover:bg-slate-800/30"} dark:border-slate-800 transition-colors`}>
                                                                    ...
                                                                    <TableCell className="border-r border-slate-200 text-center font-medium dark:border-slate-800">
                                                                        {child.material_code
                                                                            ? <Badge className="border-none bg-green-100 px-1 tracking-wider text-green-700 text-[10px] uppercase dark:bg-green-500/20 dark:text-green-400">{child.material_code}</Badge>
                                                                            : (child.category || "Khác")
                                                                        }
                                                                    </TableCell>

                                                                    <TableCell className="border-r border-slate-200 pl-8 text-slate-700 dark:border-slate-800 dark:text-slate-400">
                                                                        <div className="relative flex flex-col gap-0.5 py-1">
                                                                            ...
                                                                            {child.original_name && child.original_name !== child.material_name && (
                                                                                <span className="ml-2 text-[10px] text-slate-400 italic">(Gốc: {child.original_name})</span>
                                                                            )}
                                                                        </div>
                                                                    </TableCell>

                                                                    <TableCell className="border-r border-slate-200 text-center text-xs text-slate-600 dark:border-slate-800 dark:text-slate-400">{child.unit}</TableCell>
                                                                    <TableCell className="border-r border-slate-200 bg-slate-50/50 text-center font-mono text-xs text-slate-400 dark:border-slate-800 dark:bg-slate-900/50">{factor !== 1 ? `/${factor}` : "-"}</TableCell>

                                                                    <TableCell className="border-r border-slate-200 bg-slate-50/50 text-right font-mono text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-400">
                                                                        {savedNorm > 0 ? Number(savedNorm).toLocaleString('en-US', { maximumFractionDigits: 6 }) : "-"}
                                                                    </TableCell>

                                                                    <TableCell className="border-r border-slate-200 bg-blue-50/20 text-right font-bold text-blue-700 dark:border-slate-800 dark:bg-blue-500/5 dark:text-blue-400">
                                                                        {Number(child.quantity).toLocaleString('en-US', { maximumFractionDigits: 6 })}
                                                                    </TableCell>

                                                                    <TableCell className="border-r border-slate-200 p-1 text-right dark:border-slate-800">
                                                                        <div className="flex h-7 items-center justify-end bg-transparent px-2 text-sm text-slate-600 dark:text-slate-400">
                                                                    ...
                                                                    <TableCell className="p-1 text-center"><Button variant="ghost" size="icon" className="h-6 w-6 text-slate-300 hover:text-red-500" onClick={() => handleDelete(child.id, child.material_name)}><Trash2 className="h-4 w-4" /></Button></TableCell>
                                                                </TableRow>
                                                            );
                                                        })}
                                                    </React.Fragment>
            ...
            <Dialog open={activeModal !== null} onOpenChange={(open) => !open && setActiveModal(null)}>
                <DialogContent className="max-h-[85vh] max-w-5xl overflow-y-auto dark:border-slate-800 dark:bg-slate-900">
                    {activeModal === 'total' && (
                        <>
                            <DialogHeader><DialogTitle className="text-center text-xl font-bold tracking-widest text-slate-800 uppercase dark:text-slate-100">Bảng Tổng Hợp Chi Phí</DialogTitle></DialogHeader>
                            <Table className="mt-4 border dark:border-slate-800">
                                <TableHeader><TableRow className="border-b-2 bg-slate-100 dark:border-slate-700 dark:bg-slate-800"><TableHead className="w-[60px] text-center font-bold dark:text-slate-200">STT</TableHead><TableHead className="text-center font-bold dark:text-slate-200">Tên chi phí</TableHead><TableHead className="w-[100px] text-center font-bold dark:text-slate-200">Đơn vị</TableHead><TableHead className="w-[200px] text-right font-bold dark:text-slate-200">Giá trị</TableHead></TableRow></TableHeader>
                                <TableBody>
                                    ...
                                    <TableRow className="bg-slate-50 dark:bg-slate-800/50"><TableCell colSpan={3} className="text-center text-base font-extrabold tracking-widest text-slate-800 uppercase dark:text-slate-100">Tổng Cộng</TableCell><TableCell className="text-right text-lg font-extrabold text-blue-700 dark:text-blue-400">{formatCurrency(TotalProject)}</TableCell></TableRow>
                                </TableBody>
                            </Table>
                        </>
                    )}
                    ...
                    {['VL', 'NC', 'M'].includes(activeModal || '') && (
                        <>
                            <DialogHeader><DialogTitle className="text-center text-xl font-bold tracking-widest text-slate-800 uppercase dark:text-slate-100">Bảng Tổng Hợp {activeModal === 'VL' ? 'Vật Liệu' : activeModal === 'NC' ? 'Nhân Công' : 'Máy Thi Công'}</DialogTitle></DialogHeader>
                            <Table className="mt-4 border dark:border-slate-800">
                                <TableHeader>
                                    <TableRow className="border-b-2 bg-slate-100 dark:border-slate-700 dark:bg-slate-800">
                                        ...
                                        <TableHead className="w-[150px] bg-blue-50 text-right font-bold text-blue-800 dark:bg-blue-900/30 dark:text-blue-300">Giá NCC (Theo ĐVT Mua)</TableHead>
                                ...
                                <TableBody>
                                    {getSummaryByCategory(activeModal!).map((mat: any, idx) => (
                                        <TableRow key={idx} className="hover:bg-muted/50 dark:border-slate-800 dark:hover:bg-slate-800/50">
                                            <TableCell className="text-center text-slate-500 dark:text-slate-400">{idx + 1}</TableCell>
                                            <TableCell className="font-medium text-slate-800 dark:text-slate-200">
                                                <div className="group relative flex items-center justify-between pr-2">
                                                    <div className="flex flex-col gap-0.5">
                                                        <span>{mat.material_name}</span>
                                                        {mat.material_code && <Badge className="w-fit border-none bg-green-100 px-1 text-green-700 text-[10px] dark:bg-green-500/20 dark:text-green-400">{mat.material_code}</Badge>}
                                                    </div>
                                                    <div className="opacity-0 transition-opacity group-hover:opacity-100">
                                                        <MaterialSelector
                                                            ...
                                                            trigger={<Button variant="ghost" size="sm" className="h-6 bg-blue-50 text-xs text-blue-600 dark:bg-blue-900/50 dark:text-blue-400">Đổi mã</Button>}
                                                        />
                                                    </div>
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell className="border-r border-l border-blue-100 bg-blue-50/30 p-1 text-right dark:border-blue-900/30 dark:bg-blue-500/5">
                                                <Input
                                                    type="number"
                                                    className="h-8 border-blue-200 bg-white text-right text-sm font-bold text-blue-700 dark:border-blue-900 dark:bg-slate-950 dark:text-blue-400"
                                    ...
                                    {getSummaryByCategory(activeModal!).length === 0 && <TableRow><TableCell colSpan={6} className="py-6 text-center text-slate-400 dark:text-slate-600">Không có dữ liệu</TableCell></TableRow>}
                                </TableBody>
                            </Table>
                        </>
                    )}
