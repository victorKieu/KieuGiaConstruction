...
import { Plus, ChevronRight, ChevronDown, MoreHorizontal, Edit, Trash2, Calendar } from "lucide-react";
...
interface TaskTreeClientProps {
    projectId: string;
    // ✅ Chấp nhận cả 2 tên props để tương thích
    initialTasks?: any[];
    tasks?: any[];
    members: any[];
    dictionaries: any;
...
// Giá trị mặc định an toàn cho Dictionary
const DEFAULT_DICTIONARIES = { statuses: [], priorities: [] };

export default function TaskTreeClient({
    projectId,
    initialTasks,
    tasks,
    members = [],
    ...
    // ✅ QUAN TRỌNG: Gộp dữ liệu từ props (ưu tiên cái nào có dữ liệu)
    const taskDataList = (Array.isArray(initialTasks) ? initialTasks : [])
        .concat(Array.isArray(tasks) ? tasks : []);

    // Nếu cả 2 đều rỗng thì fallback về mảng rỗng, tránh undefined
    const safeTaskList = taskDataList.length > 0 ? taskDataList : (initialTasks || tasks || []);
    ...
    const taskTree = useMemo(() => {
        if (!Array.isArray(safeTaskList)) return [];

        const map = new Map<string, Task>();
        const roots: Task[] = [];

        // Lọc trùng lặp nếu có
        const uniqueTasks = Array.from(new Map(safeTaskList.map(item => [item.id, item])).values());
    ...
    const renderRow = (task: Task, level: number = 0): React.ReactNode => {
        ...
        return (
            <>
                <TableRow key={task.id} className="hover:bg-slate-50 group border-b border-slate-100">
                    <TableCell className="py-3">
                        <div className="flex items-center" style={{ paddingLeft: `${paddingLeft}px` }}>
                            ...
                            <span className="font-medium truncate max-w-[300px] text-slate-800" title={task.name}>{task.name}</span>
                        </div>
                    </TableCell>
                        ...
                        {task.assignee ? (
                            <div className="flex items-center gap-2">
                                {task.assignee.avatar_url ? (
                                    <img src={task.assignee.avatar_url} className="w-6 h-6 rounded-full object-cover border" />
                                ) : (
    ...
    return (
        <div className="border rounded-lg bg-white shadow-sm overflow-hidden">
            <Table>
                ...
                <TableBody>
                    {taskTree.length === 0 ? <TableRow><TableCell colSpan={6} className="text-center py-12 text-slate-500">Chưa có dữ liệu.</TableCell></TableRow> : taskTree.map(task => renderRow(task))}
                </TableBody>
            </Table>
            ...
            <TaskCreateModal
                ...
                tasks={safeTaskList} // Truyền dữ liệu an toàn
            />
        </div>
    );
}