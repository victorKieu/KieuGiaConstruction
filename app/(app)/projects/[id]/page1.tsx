...
import { ArrowLeft, Edit, Clock, DollarSign, ListTodo, ShieldAlert } from "lucide-react";
import { formatDate } from "@/lib/utils/utils";

// Component chính của trang động /projects/[id]
// Đây là Server Component, và nó là async để nhận params
export default async function ProjectDetailPage({ params }: { params: { id: string } }) {
    const id = params.id; // ✅ dùng sau khi nhận params trong async component
...
async function ProjectDetail({ id }: { id: string }) {
    ...
    const getConstrutionTypeBadge = (construction_type: string | null) => {
        switch (construction_type) {
            case "new":
                return <Badge className="bg-blue-500">Xây mới</Badge>;
            case "repair":
                return <Badge className="bg-green-500">Sửa chữa</Badge>;
            case "renovation":
                return <Badge className="bg-yellow-500">Cải tiến</Badge>;
            case "expansion":
                return <Badge className="bg-purple-500">Mở rộng</Badge>;
            default:
                return <Badge className="bg-gray-500">Không xác định</Badge>;
        }
    };

    const getProjectTypeBadge = (project_type: string | null) => {
        switch (project_type) {
            case "building":
                return <Badge className="bg-blue-500">Công trình dân dụng</Badge>;
            case "industrial":
                return <Badge className="bg-green-500">Công trình công nghiệp</Badge>;
            case "infrastructure":
                return <Badge className="bg-yellow-500">Hạ tầng kỹ thuật</Badge>;
            case "transport":
                return <Badge className="bg-purple-500">Giao thông</Badge>;
            case "urban":
                return <Badge className="bg-purple-500">Đô thị</Badge>;
            case "agriculture":
                return <Badge className="bg-purple-500">Nông nghiệp/Nông thôn</Badge>;
            case "other":
    ...
    const getRiskBadge = (risk: string | null) => {
        switch (risk) {
            ...
            case "medium":
                return <Badge className="bg-yellow-500">Có thể ảnh hưởng tiến độ</Badge>;
            case "high":
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <Card>
                    <CardContent className="p-4 flex items-center">
                        ...
                        <div>
                            <p className="text-sm text-gray-500">Thời gian thi công</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <Card>
                    <CardContent className="p-4">
                        ...
                        <p className="text-sm text-gray-700 mb-1">
                            Loại dự án: {getProjectTypeBadge(project.project_type)}
                        </p>
                        <p className="text-sm text-gray-700 mb-1">
                            Loại công trình: {getConstrutionTypeBadge(project.construction_type)}
                        </p>
                        <p className="text-sm text-gray-700 mb-1">
                ...
                <Card>
                    <CardContent className="p-4">
                        ...
                        <p className="text-sm text-gray-700 mb-1">
                            Vị trí: {project.location || "Chưa có định vị"}
                        </p>
                    </CardContent>
                </Card>
            </div>
